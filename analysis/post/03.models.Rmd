...
# plot_predictions(m12bfi, vcov=TRUE, condition=c('bfi_agreeableness', 'diagnosis_subtype'))  + labs(x='Agreeableness', y='Rating', fill="Diagnosis Subtype", color='Diagnosis Subtype') + theme_light(base_family='Lato')
...
variables <- c("bfi_agreeableness", "bfi_extraversion", "bfi_openness", "bfi_conscientiousness", "bfi_neuroticism")
...
# Optional: define shared y-axis limits
y_limits <- c(3, 7)  # adjust to fit your scale

plots <- map(variables, function(var) {
  plot_predictions(
    m12bfi,
    by = var,
    group = "diagnosis_subtype"
  ) +
    labs(x = var_labels[[var]], y = "Rating", color = "Diagnosis Subtype") +
    coord_cartesian(ylim = y_limits) +  # ⬅️ fixes shared y-axis visually
    theme_minimal()
})

# Combine with patchwork (shared guide and layout)
bfi_plot <- wrap_plots(plots, ncol = 2, guides = "collect") +
  plot_annotation(
    title = "Comfort Ratings by BFI-10\nAverage Marginal Predictions",
...
ggsave(
  filename = "plots/expl_bfi10_marginal_predictions.eps",
  plot = bfi_plot,
  device = "eps",
  width = 10,
  height = 6,
...
### Trust and Help-seeking Behavior
